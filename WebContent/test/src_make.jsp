...
<sp:sp queryPath="${param.queryPath }" action="${param.action }" processorList="db" exception="false">
	{
		${param.defaultValue }
	}
</sp:sp> 
...
<c:forEach var="map" items="${JSON }">
	<c:if test="${fn:endsWith(map.key, '_meta_') }">
		...
		<c:forEach var="fld" items="${JSON[map.key] }">
			<c:set var="info" value="${fld.value }"/>
			<tr>
				<td label="${info.name}" title='${info }'>${info.label }</td>
				<c:set var="label">${info.name}_label</c:set>
				<c:set var="type">${info.name}_type</c:set>
				<c:set var="edit">${info.name}_edit</c:set>
				<c:set var="link">${info.name}_link</c:set>
				<c:set var="valid">${info.name}_valid</c:set>
				<c:set var="keyValid">${info.name}_key_valid</c:set>
				<td><input type="text" name="${label}" style="width: 100%" value="${empty(param[label]) ? info.label : param[label] }"></td>
				<td>
...
<c:set var="src">
		...
		<c:forEach var="map" items="${JSON }">
			<c:if test="${fn:endsWith(map.key, '_meta_') }">
				<c:set var="rowId" value="${fn:replace(map.key, '_meta_', '') }"/>
				
				<c:if test="${param.view_type=='list' }"><%//리스트인 경우 %>
					${'<' }c:forEach var="row" items="${'$'}{${rowId } }" varStatus="status">
						<tr class="row_${'$' }{status.index + 1}">
							<c:forEach var="fld" items="${JSON[map.key] }" >
								...
								<c:set var="valid">${info.name}_valid</c:set>
								<c:set var="keyValid">${info.name}_key_valid</c:set>
								<c:set var="title">${title }<th label="${info.name}">${param[label] }</th></c:set>
	...
	function link_${info.name }(obj){
		...
		alert(${info.name});
								...
								<td><tag:fild src="${info }" name="${info.name }" type="${param[type] }" edit="${param[edit] }" link="${param[link] }" index="row_${'$' }{status.index + 1}" valid="${param[valid] }"  keyValid="${param[keyValid] }" /> </td>
							</c:forEach>
						</tr>
					${'<' }/c:forEach>
					<c:set var="title"><tr>${title }</tr></c:set>
				</c:if>
				<c:if test="${empty(param.view_type)}">	<%//상세페이지인 경우%>
				
					${'<' }c:set var="row" value="${'$'}{${rowId} }"/>
					<colgroup>
						<col width="150">
						<col width="*">
					</colgroup>
	...
	function link_${info.name }(obj){
		...
		alert(${info.name});
						...
						<tr>
							<th label="${info.name}">${param[label] }</th>
							<td><tag:fild src="${info }" name="${info.name }" type="${param[type] }" edit="${param[edit] }" link="${param[link] }" valid="${param[valid] }"  keyValid="${param[keyValid] }" /> </td>
						</tr>
					</c:forEach>
				</c:if>
			</c:if>
...
&lt;sp:sp queryPath="${param.queryPath }" action="${param.action }" processorList="attach,db" exception="false">
	{
		//${param.defaultValue }
	}
...
<div id="main_layer" style="margin: 0 auto; padding:3px; width: 90%; min-width:1000px; border:1px solid #cccccc;">
	...
	<table class="${empty(param.view_type) ? 'vw' : 'lst' }" border="0" cellspacing="0" cellpadding="0" >
		${title }<%//목록타입인 경우 컬럼의 제목 %>
		${src }
	</table>
	</form>
