...
<c:forEach var="row" items="${rows }">
	<table>
		<tr id="${row.menu_id }" upp_menu_id="${row.upp_menu_id }" class="${row.upp_menu_id } sub_menu menu_load jqgfirstrow" dep="${req.dep+1}">
			...
			<td align="center"><input type="checkbox" name="acc_read" class="btn_acc"  menu_id="${row.menu_id }" ${row.acc_read=='N' ? '' : 'checked=true' }></td>
			<td align="center"><input type="checkbox" name="acc_save" class="btn_acc"  menu_id="${row.menu_id }" ${row.acc_save=='N' ? '' : 'checked=true' }></td>
			<td align="center"><input type="checkbox" name="acc_excel" class="btn_acc"  menu_id="${row.menu_id }" ${row.acc_excel=='N' ? '' : 'checked=true' }></td>
		</tr>
	</table>	
