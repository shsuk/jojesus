...
<c:choose>
	...
	<c:otherwise>
		...
		<c:choose>
			<c:when test="${type=='select'}">${'$'}{code:name('${name }', ${src_id}['${name}'],null)}</c:when>
			<c:when test="${type=='date'}">${'$'}{${src_id}['${name }@yyyy-MM-dd']}</c:when>
			<c:when test="${type=='number'}">${'$'}{${src_id}['${name }@#,##0']}</c:when>
			<c:otherwise>${'$'}{${src_id}.${name }}</c:otherwise>
		</c:choose>
		</span>
	</c:otherwise>
</c:choose>
