...
<c:forEach var="row" items="${code:list(groupId)}"> 
	<input name="${name}" id="${name}_${row.code_value }"  type="radio" value="${row.code_value }" ${att } ${checked == row.code_value ? 'checked' : ''} ${valid } title="${title }" style="float: left;"  ${attr } ><label style="float: left;line-height: 20px;" for="${name}_${row.code_value }">${code:name(groupId, row.code_value, lang)}&nbsp;&nbsp;</label>
</c:forEach>