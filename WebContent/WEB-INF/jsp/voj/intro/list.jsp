...
<c:if test="${req.action=='d' }">
	<uf:organism noException="true">[
		<job:db id="row" query="voj/intro/delete"/>
	]</uf:organism>	
</c:if>

<uf:organism >
[
	<job:db id="rows" query="voj/intro/list" singleRow="false" >
		defaultValues:{
...
<script type="text/javascript">
	...
	function edit(bd_id){
		...
		var url = 'at.sh';
		var data = {_ps:'voj/intro/edit', bd_id: bd_id,  _layout: 'n', pageNo: (bd_id ? '${empty(req.pageNo) ? 1 : req.pageNo}' : '1')};
	...
	function del(bd_id){
		...
		var url = 'at.sh';
		var data = {_ps:'voj/intro/list', bd_id: bd_id,action:'d', _layout: 'n', pageNo: (bd_id ? '${empty(req.pageNo) ? 1 : req.pageNo}' : '1')};
