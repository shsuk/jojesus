...
public class RSATest {
	public static final String RSA = "ECDSA";
	public static final String RSA1 = "EC";
	
	public static void main(String[] args) throws NoSuchAlgorithmException {
		KeyPairGenerator kpg = KeyPairGenerator.getInstance("SHA256withECDSA", new BouncyCastleProvider());
		  kpg.initialize(2048);
		  KeyPair keyPair = kpg.generateKeyPair(); //키 쌍 생성
		  
		  PublicKey publicKey = keyPair.getPublic(); //공개키 생성
		  PrivateKey privateKey = keyPair.getPrivate(); //개인키 생성

		
		
		
		encrypt("dfdfdfdfdf", publicKey);
	}
	
	public static KeyPair generateKeyPair() {
		KeyPair keyPair = null;
		try {
			keyPair = KeyPairGenerator.getInstance("SHA256withECDSA", new BouncyCastleProvider()).generateKeyPair();
	...
	public static String encrypt(String text, PublicKey publicKey) {
		...
		try {
			Cipher cipher = Cipher.getInstance("SHA256withECDSA", new BouncyCastleProvider());
	...
	public static RSAPrivateKeySpec getRSAPrivateKeySpec(PrivateKey privateKey) {
		...
		try {
			spec = KeyFactory.getInstance("SHA256withECDSA", new BouncyCastleProvider()).getKeySpec(privateKey, RSAPrivateKeySpec.class);
		} catch (InvalidKeySpecException e) {
			e.printStackTrace();
		} catch (NoSuchAlgorithmException e) {
			e.printStackTrace();
	...
	public static PublicKey getPublicKey(String modulus, String exponent) {
		...
		try {
			publicKey = KeyFactory
					.getInstance("SHA256withECDSA", new BouncyCastleProvider())
